...
public class ModelUnit extends Node {
    ...
    private List<OpenDecl> createOpenDeclList(List<CompModule.Open> openDecls) {
        return openDecls.stream().map(openDecl -> new OpenDecl(this, openDecl)).collect(Collectors.toList());
    }

    private List<SigDecl> createSigDeclList(List<Sig> sigDecls) {
        return sigDecls.stream().map(sigDecl -> new SigDecl(this, sigDecl)).collect(Collectors.toList());
    }

    private List<Fact> createFactDeclList(ConstList<Pair<String,Expr>> factDecls) {
        return factDecls.stream().map(factDecl -> new Fact(this, factDecl)).collect(Collectors.toList());
    }

    private List<Assertion> createAssertDeclList(ConstList<Pair<String,Expr>> assertDecls) {
        return assertDecls.stream().map(assertDecl -> new Assertion(this, assertDecl)).collect(Collectors.toList());
    }
